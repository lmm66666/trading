...
type ComputeRunModel struct {
	BaseModel
	RunID             string     `gorm:"size:64;not null;uniqueIndex:uq_compute_run"`
	...
	IdempotencyKey    string     `gorm:"size:128;not null;uniqueIndex:uq_compute_idempotency,priority:2"`
	...
	LeaseToken        string     `gorm:"size:128;not null"`
