...
应用层编排行情采集与查询、图表、证券搜索、扫描、回测、持久化 Worker 和调度生命周期。它负责锁定可复现输入、协调端口、分类重试与取消，并把领域结果交给原子发布边界。

应用层只依赖领域值对象与 port，不导入 GORM、Gin、具体 Broker 或旧 K 线模型；业务算法留在领域模块，外部协议解析留在适配器。
