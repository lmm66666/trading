...
| `api` | HTTP 校验、应用调用、响应映射、兼容适配 | application、只读 port、Gin | [API 设计](../../api/DESIGN.md)、[HTTP 契约](../../api/api.md) |
| `business` | 财报、宏观查询、信号与旧调度业务 | data、model、broker、financialscreen | [业务设计](../../business/DESIGN.md) |
| `data` / `model` | 财报和证券主数据访问；旧 K 线迁移/回滚模型 | GORM、model | [旧数据访问设计](../../data/DESIGN.md) |
...
| `internal/backtest` | 账户、撮合、费用、公司行动、结果和指标 | market、strategy | [回测设计](../../internal/backtest/DESIGN.md) |
| `internal/application` | 行情、图表、扫描、回测和 Worker 用例编排 | port 与领域模块 | [应用层设计](../../internal/application/DESIGN.md) |
| `internal/financialscreen` | 财报筛选规则与组合 | model 中的财报值 | [财报筛选设计](../../internal/financialscreen/DESIGN.md) |
| `internal/port` | 应用所需的存储、数据源、任务与遥测边界 | 领域对象、标准库 | [端口设计](../../internal/port/DESIGN.md) |
| `internal/infrastructure/mysql` | 版本化行情、任务、租约、结果和迁移持久化 | port、market、GORM/MySQL | [MySQL 设计](../../internal/infrastructure/mysql/DESIGN.md) |
| `pkg/broker` | 新浪与东方财富外部数据源适配 | port、market、HTTP | [Broker 设计](../../pkg/broker/DESIGN.md) |
| `pkg/indicator` | 旧财报指标辅助和共享上游限频器 | 标准库 | 归入 [Broker 设计](../../pkg/broker/DESIGN.md) 的限频边界与 [业务设计](../../business/DESIGN.md) 的旧指标边界 |
| `web` | React 行情工作台和 API 客户端状态 | React、Lightweight Charts、HTTP API | [前端设计](../../web/DESIGN.md) |
| `cmd/migrate-strategy-kernel` | 旧行情到版本化内核的一次性迁移 | data/model、broker、application、MySQL adapter | [迁移设计](../../cmd/migrate-strategy-kernel/DESIGN.md) |
...
- 发现模块职责与本地图不符时，先修订地图和相关设计，再继续实现。
