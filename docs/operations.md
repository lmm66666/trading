...
完整脚本包含覆盖率、Race Detector、静态检查、5000 证券性能、ARM64 原生 MySQL 8.4 集成和 `linux/amd64` 应用镜像构建。Docker 不可用时必须准确报告未完成的步骤，不能用 SQL mock 替代真实事务与并发验证。

本地集成测试不得强制 MySQL 容器使用 amd64；官方 `mysql:8.4` 多架构标签应由 Docker 解析为宿主原生镜像。部署数据库兼容检查只允许读取版本和编译架构，凭据通过当前进程环境注入，不写入仓库、日志或命令示例。
