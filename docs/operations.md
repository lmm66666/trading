...
本地调试时将两个服务指向本地 mock 库，URL 改为 `http://127.0.0.1:8081`；mock 库与 NAS 库独立，不自动同步或回退切换。updater 启动后先等待首个刷新周期；需要立即更新行情时，通过 workbench 的手动刷新接口触发股票与已启用期货。纯查看 mock 数据也可只启动 workbench，届时手动刷新会显示更新服务不可达。
...
- 股票启动满 24 小时后首次自动更新，再按原 ticker 更新，仍重抓最近 20 根日线；期货启动后等待配置的刷新间隔，沿用固定八品种和全历史刷新。重启重新计时。全市场手动刷新同时尝试股票与已启用期货，已运行类别不重复启动，另一类照常启动，且不重置定时节拍。
...
工作台顶栏“数据更新”通过一个“立即更新”按钮更新全市场股票与已启用期货，分别查看受理结果、进度与失败记录。关闭电脑不会停止 NAS 任务，重启 updater 后旧任务记为中断、不自动续跑。重新更新仍按原行情规则遍历，并不跳过当天已更新证券。progress_available=false 表示采集已受理但记录暂不可用，不应自动重复提交。进度记录可能落后真实行情，不把百分比当数据库完整性证明。
...
统一批量接口返回两类回执，升级时前端、workbench 与 updater 必须同步更新，不兼容旧股票批量回执。期货未启用会明确跳过；空进度记录正常显示“暂无更新记录”，不代表更新服务已连通。状态请求约两秒且期货启用状态未知时，应检查 updater 网络、端口与 Token；该等待来自能力查询超时，不是空记录 SQL 失败。
