...
完整脚本包含覆盖率、Race Detector、静态检查、5000 证券性能、远端 MySQL 8.4 随机隔离数据库集成和 `linux/amd64` 应用镜像构建。缺少远端测试凭据或 Docker 时必须准确报告未完成步骤，不能用 SQL mock 替代真实事务与并发验证。

本地不得启动或拉取 MySQL 作为验收环境。远端预检只读取版本和编译架构；完整集成测试为每个测试创建 `trading_test_` 前缀的随机空数据库，结束后删除。凭据通过 `TRADING_TEST_MYSQL_DSN` 注入，不写入仓库、日志或命令示例；缺少变量时门禁失败。
