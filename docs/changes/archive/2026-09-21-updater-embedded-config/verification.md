...
- 原全量镜像检查因外部依赖下载停滞中止，未记录为通过。用户随后批准按风险简化验收；最终独立执行 `--image=updater-configured` 与 `--image=updater` 通过，分别见 `/tmp/verify-efficient-configured.log` 和 `/tmp/verify-efficient-updater.log`。workbench 未改变，无需重建；实际 NAS 部署不在本地验收中冒充通过。
...
- REQ-EC-002：实际普通 updater 镜像检查不包含 /app/config.yaml；同一路径连续两次更换 secret 内容，镜像逐字节匹配新内容；不提供 secret 且强制刷新配置阶段时构建失败。均通过。
