...
- `/api/v1` 提供证券搜索、策略目录、图表、版本化行情、回测任务、扫描任务和不可变快照。
- `/api/stocks/*` 保留两类兼容入口：行情、信号和回测路由适配新内核；财报、宏观及其调度路由继续调用 `business` 兼容服务。
- Web 工作台与外部客户端共享统一 JSON 包装 `{code,message,data}`。
- 同源静态资源由 `serve_web` 托管，开发模式由 Vite 代理 API。

## 3. 依赖和边界

- V1 handler 通过 `KernelServices` 注入应用服务和只读 port。
- handler 只完成“解析 → Validate → 调用 → 映射”，所有调用传递请求 context。
- 兼容接口可以做旧参数到完整领域身份的适配，但不能复制业务实现。
...
- `/api/v1` 创建请求使用显式 DTO 和 Validate，JSON body 上限 1MiB，拒绝未知字段、尾随第二个 JSON 值和重复查询参数。
- `/api/stocks/*` 兼容创建接口仍使用 Gin `ShouldBindJSON` 和各 handler 的既有字段校验，不承诺 V1 的 1MiB、未知字段或尾随值规则。该例外只用于维持当前兼容行为，新接口不得复制。
- V1 时间按 RFC3339 接收并规范化为 UTC；日期范围、分页 limit、身份字节长度和指标预算在进入应用层前按契约校验。
- 旧财报分页的 `pagesize` 和财报信号的 `quarter_count` 当前只校验为正数，没有最大值；这是现行兼容边界，不得复制到新接口。统一资源上限需要大型需求和兼容方案。
- 证券优先使用完整 `Exchange:Code` 身份；旧六位代码只精确匹配活跃证券，零、一或多结果分别映射为稳定语义。
...
图表查询在应用层完整历史上下文计算指标后返回裁剪页；API 只保持请求顺序、版本和游标。兼容信号只读一个完整已发布快照，兼容回测创建持久化 Run 并在有界时间内等待。
...
- V1 以及已迁移的新内核兼容接口对 body、页大小、证券范围、日期范围、指标数量和计算成本设置硬上限；旧财报兼容接口仍存在上一节记录的无上限参数。
- Handler 日志不得记录完整请求/响应、幂等键、Cookie、Token 或内部错误正文。
- 批量扫描和行情读取必须调用应用层批量能力，API 不按证券循环查询 Repository。

## 8. 测试与验收证据

测试覆盖严格 JSON、请求限制、错误脱敏、context 传播、身份解析、幂等冲突、固定版本图表、快照分页、兼容切换和前端托管。
