...
读取结果时，结果行与失败项附带读取时点 `t_instruments` 的当前证券名称 `name`：它是显示属性，不随快照固化，证券更名后旧快照按新名称显示；名称缺失时字段省略，客户端回退显示证券代码。一份快照内每只证券最多一行（见下表唯一约束），同一快照内各行信号时间通常相同（窗口最后一根 Bar 收盘），停牌证券可能更早。
