...
支持的指标：`SMA`、`EMA`、`STD`（单序列，参数 `period`）、`MACD`（`fast`/`slow`/`signal`，返回 DIF、DEA、Histogram 三条）、`KDJ`（`period`，返回 K、D、J 三条），以及 `RETZ`（涨幅偏差，返回柱、平滑、长期三条）。
...
为防止一次请求算爆，有两条预算：指标成本总和 ≤ 2000（SMA/EMA 每个记 1，MACD 记 3，KDJ 记 `period × 3`，STD 记 period，RETZ 记 `period + regime`），以及进程内同时进行的指标构建最多 4 个，超出的请求排队等待。重复参数组合（同 kind 同参数）的指标会被拒绝而不是去重。
...
添加“涨幅Z 126,5,252”后，柱子表示当天对数涨幅偏离最近126根涨幅均值多少个总体标准差；浅色线是柱子的5期 EMA，紫色线按252根窗口标准化同一涨幅。±1σ、±2σ参考线帮助识别偏离程度，零波动或历史不足时留空。切换周线后每根输入变成周涨幅；它衡量单标的变化，不是股票与油价之间的相对价差。完整契约见[指标设计](../internal/indicator.md#每根涨幅偏差-retz)。

RETZ 扩展的实现与测试证据见[变更验证](../../changes/archive/2026-09-21-return-zscore/verification.md)；上方 baseline_revision 保留原业务说明的历史基线。
